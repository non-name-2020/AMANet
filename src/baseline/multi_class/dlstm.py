    ...
    if TEST:
        ...
        for epoch in range(EPOCH):
            ...
            for sample_index in range(train_size):
                ...
                target = output
                pred_prob = model(input)
                loss = F.cross_entropy(pred_prob, torch.FloatTensor(target).long())
            ...
            print("test acc: %.4f, prec: %.4f, recall: %.4f, f1: %.4f, prauc: %.4f, roc_auc: %.4f\n" % (acc, prec, recall, f1, prauc, roc_auc))
            print("###############################################################\n")
